...
#block(
  ...
  *首次使用最短路径*：上电 → 连接配置器 -> 检查是否需要更新固件 -> 设置 ELRS 对频密码 → 导入预设配置 。
]
...
第一次使用建议点击新手引导，辅助进行配置。
#image("assets/image-5.png")

浏览社区配置，选择合适的构型导入。
#image("assets/image-6.png")

根据提示完成水平面校准。注意固定翼飞机水平面校准时，可稍微抬头，模拟飞机平飞姿态。
#image("assets/image-7.png")

这样飞控设置即可完成。

=== 感度调节
在飞控页面，可以进行感度（控制强度）调节，调节范围为1-10。数字越大，控制越灵敏。

=== 反馈方向
当飞机解锁后，切换到自稳模式（角度模式），正确的控制方向为：
- 飞机低头，升降舵上翘；飞机抬头，升降舵下压。

- 飞机左滚，右副翼上翘；飞机右滚，左副翼上翘。

- 飞机左偏航，方向舵右偏；飞机右偏航，方向舵左偏。
（注意，由于偏航轴只能进行增稳（角速度）控制，因此只有飞机偏航的瞬间，方向舵会有反应）

如果你的飞机在自稳模式下，控制方向与上述不一致，可以在飞控页面，勾选对应轴的反馈反向。
#image("assets/image-8.png")
=== 舵机去抖
如果飞起来之后，感觉舵机抖动明显，可以在飞控页面，勾选对应轴的舵机去抖。


=== 什么时候需要用专家模式
当前配置器默认使用新手模式，提供了最常用的功能和设置。对于有经验的用户，可以切换到专家模式，进行更细致的参数调节。

常见需要使用专家模式的情况有：
- 需要使用电调（因为默认的配置都是用有刷电机，如果要用电调，需要切换到专家模式，设置PWM输出为50-400hz，并勾选极性反转(前四个通道)）
