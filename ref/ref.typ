...
== 核心技术亮点
- *极简交互*：采用工业级串口转换芯片，确保手机与射频模块之间的串行通讯稳定如磐，无丢包风险。
- *开源力量*：基于著名的 ExpressLRS (ELRS) 开源项目，内置高性能处理芯片。ELRS 是目前航模界最先进的无线协议之一，以超高的刷新率和极强的抗干扰能力著称。
- *供电无忧*：设备直接从手机 Type-C 接口获取电力，无需外接笨重的动力电池即可驱动射频工作，真正做到即插即用。
- *全能补给*：除了作为发射机，它还集成了一套专用于 1s 航模电池的充电系统，可以为常见的 1S 飞控电池提供高达 500mA 的恒流充电。
...
#placeholder("硬件接口与按键分布标注图", img_path: "2.png")
...
#placeholder("电池接口连接示意图", img_path: "3.png")
3. *内置 ELRS 天线*
...
#placeholder("指示灯分布示意图", img_path: "4.png")
#table(
...
== 第二步：物理互联流程
#placeholder("手机与转换器连接示意图", height: 12em, img_path: "5.png")

将转换器插入手机。如果您的手机预先下载安装了官方的App，这里会弹出相关的提示，引导您打开Let's Fly App。

== 第三步：操控配置与校准
#placeholder("Let's Fly app 界面布局图", height: 12em, img_path: "6.png")
...
#placeholder("ELRS 配置后台界面截图", img_path: "7.png")
...
#placeholder("WiFi配置页在线烧录固件示意图", img_path: "8.png")
